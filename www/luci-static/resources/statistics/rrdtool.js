	...
	_generic: function(opts, host, plugin, plugin_instance, dtype, index) {
		...
		function __line(source) {
			...
			if (typeof(source.color) == 'string') {
				...
				color = colors.asString(colors.random());
			}

			/* if not disabled, create area statement */
			if (!source.noarea) {
				_args.push('AREA:%s_%s#%s:%s'.format(
					source.sname,
					source.flip ? 'neg' : 'plot',
					source.area_color || colors.asString(colors.faded(color)),
					'%%-%us'.format(_longest_name).format(source.title)
				));
			}

			/* if not disabled, create line statement */
			if (!source.noline) {
				_args.push('LINE%d:%s_%s#%s:%s'.format(
					source.line_width || (source.noarea ? 2 : 1),
					source.sname,
					source.flip ? 'neg' : 'plot',
					color,
					( source.noarea ? '%%-%us'.format(_longest_name).format(source.title) : '' )
				));
			}
		}
			...
			for (var j = 0; j < data_instances.length; j++) {
				...
				for (var k = 0; k < data_sources.length; k++) {
					...
					var source = {
						...
						/* line and area display */
						color: dopts.color || false,
						line_width: dopts.line_width || false,
						noline: dopts.noline || false,
						area_color: dopts.area_color || false,
